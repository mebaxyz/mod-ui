...
from servicebus import Service, handler, event_handler, get_config, set_config, CommConfig
...
class ExampleEffectsService:
    ...
    def _register_handlers(self):
        ...
        self.service.register_handler("get_plugins", self.get_plugins, "Get all available plugins")
        self.service.register_handler("get_plugin_info", self.get_plugin_info, "Get info for a specific plugin")
    ...
    async def load_plugin(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await self.service.publish_event("plugin.loaded", {
            "uri": plugin_uri,
...
class ExampleWebUIGateway:
    ...
    def _register_handlers(self):
        ...
        self.service.register_handler("api_effect_list", self.api_effect_list, "API: Get effects list")
        self.service.register_handler("api_effect_info", self.api_effect_info, "API: Get effect info") 
        self.service.register_handler("api_effect_load", self.api_effect_load, "API: Load effect")
...
class ExampleWithDecorators:
    ...
    async def calculate(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await self.service.publish_event("calculation.performed", {
            "operation": operation,
            "a": a,
            "b": b,
            "result": result
...
async def run_smart_call_example():
    ...
    service.register_handler("greet", lambda data: {"message": f"Hello, {data.get('name', 'World')}!"})
    ...
    try:
        ...
        result1 = await service.smart_call("greet", {"name": "Alice"}, prefer_local=True)
