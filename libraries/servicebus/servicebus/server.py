...
class ServiceServer:
    ...
    def __init__(self, service_name: str, redis_client: Optional[redis.Redis] = None):
        ...
        self._health_status = ServiceHealth(
            service_name=service_name, status="starting", timestamp=datetime.utcnow()
        )
    ...
    async def _register_builtin_handlers(self) -> None:
        ...
        self.register_handler(
            "health_check", self._handle_health_check, "Get service health status"
        )
        ...
        self.register_handler(
            "ping", self._handle_ping, "Simple ping/pong for connectivity testing"
        )
    ...
    async def _send_response(
        ...
        redis_client = await self.get_redis()
        response_channel = (
            f"service:{request.source_service}:responses:{request.request_id}"
        )
...
class ServiceServerBuilder:
    ...
    def register_handlers_from_class(
