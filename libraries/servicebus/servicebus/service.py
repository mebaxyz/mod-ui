...
from .models import RequestMetrics, ServiceEvent, ServiceHealth
...
class Service:
    ...
    def __init__(
        ...
        self._event_publisher = (
            EventPublisher(service_name, self._event_bus) if enable_events else None
        )
    ...
    async def start(self) -> None:
        ...
        if self._is_running:
            logger.warning("Service '%s' is already running", self.service_name)
            return

        logger.info("Starting microservice '%s'...", self.service_name)
        ...
        try:
            ...
            for task in self._startup_tasks:
                try:
                    ...
                    logger.error("Error in startup task: %s", e)
            ...
            logger.info("Microservice '%s' started successfully", self.service_name)

        except Exception as e:
            logger.error("Failed to start microservice '%s': %s", self.service_name, e)
            await self.stop()
            raise
    ...
    async def stop(self) -> None:
        ...
        if not self._is_running:
            return
        logger.info("Stopping microservice '%s'...", self.service_name)
        ...
        try:
            ...
            for task in self._shutdown_tasks:
                try:
                    ...
                    logger.error("Error in shutdown task: %s", e)
            ...
            logger.info("Microservice '%s' stopped", self.service_name)

        except Exception as e:
            logger.error("Error stopping microservice '%s': %s", self.service_name, e)
    ...
    async def smart_call(
        ...
        if prefer_local and request_type in self._server._handlers:
            try:
                ...
                logger.warning("Local handler failed for %s: %s", request_type, e)
...
def create_service(
    service_name: str, redis_url: Optional[str] = None, **kwargs
) -> Service:
    """
