...
class MetricsCollector:
    ...
    async def record_request_metric(self, metric: RequestMetrics) -> None:
        ...
        await redis_client.setex(
            metric_key, self.config.metrics_retention_seconds, metric.json()
        )

    async def get_service_metrics(
        self, service_name: str, time_range_seconds: Optional[int] = None
    ) -> Dict[str, Any]:
    ...
    async def get_system_metrics(self) -> Dict[str, Any]:
        ...
        cutoff_time = datetime.utcnow() - timedelta(
            seconds=self.config.metrics_retention_seconds
        )
    ...
    def _aggregate_metrics(self, metrics: List[RequestMetrics]) -> Dict[str, Any]:
        ...
        for metric in metrics:
            ...
            if metric.success:
                rt["success"] += 1
            rt["avg_time"] = (
                rt["avg_time"] * (rt["count"] - 1) + metric.response_time_seconds
            ) / rt["count"]
    ...
    async def export_metrics(
        self, format_type: str = "json", time_range_seconds: Optional[int] = None
    ) -> str:
        """
    ...
    def _to_prometheus_format(self, metrics: Dict[str, Any]) -> str:
        ...
        lines.append(
            f"microservice_successful_requests {metrics['successful_requests']}"
        )
        ...
        for service_name, service_metrics in metrics.get("services", {}).items():
            ...
            lines.append(
                f"microservice_service_requests{{{service_label}}} {service_metrics['total_requests']}"
            )
            lines.append(
                f"microservice_service_success_rate{{{service_label}}} {service_metrics['success_rate']}"
            )
            lines.append(
                f"microservice_service_avg_response_time{{{service_label}}} {service_metrics['avg_response_time']}"
            )
    ...
    def _to_csv_format(self, metrics: Dict[str, Any]) -> str:
        ...
        lines = [
            "service,total_requests,successful_requests,failed_requests,success_rate,avg_response_time,p95_response_time"
        ]

        # System-wide row
        lines.append(
            f"SYSTEM,{metrics['total_requests']},{metrics['successful_requests']},{metrics['failed_requests']},{metrics['success_rate']:.3f},{metrics['avg_response_time']:.3f},{metrics['p95_response_time']:.3f}"
        )

        # Per-service rows
        for service_name, service_metrics in metrics.get("services", {}).items():
            lines.append(
                f"{service_name},{service_metrics['total_requests']},{service_metrics['successful_requests']},{service_metrics['failed_requests']},{service_metrics['success_rate']:.3f},{service_metrics['avg_response_time']:.3f},{service_metrics['p95_response_time']:.3f}"
            )
