...
from .models import ServiceHealth, ServiceRegistration
...
class ServiceDiscovery:
    ...
    async def wait_for_service(
        ...
        raise TimeoutError(
            f"Service '{service_name}' did not become available within {timeout} seconds"
        )
    ...
    async def get_service_statistics(self) -> Dict[str, any]:
        ...
        healthy_services = sum(
            1 for reg in services.values() if reg.health.status == "healthy"
        )
...
class ServiceLoadBalancer:
    ...
    async def get_service_for_request(
        self, request_type: str, strategy: str = "round_robin"
    ) -> Optional[str]:
        """
        ...
        healthy_services = [reg for reg in services if reg.health.status == "healthy"]
        ...
        if strategy == "round_robin":
            ...
            best_service = min(
                healthy_services,
                key=lambda reg: reg.health.error_count
                / max(reg.health.request_count, 1),
            )
    ...
    def _round_robin_select(
        self, request_type: str, services: List[ServiceRegistration]
    ) -> str:
