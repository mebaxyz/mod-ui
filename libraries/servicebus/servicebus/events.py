...
class EventBus:
    ...
    async def publish(self, event: ServiceEvent) -> int:
        ...
        if self.config.debug_mode:
            logger.info(
                "Published event %s from %s to %s subscribers",
                event.event_type,
    ...
    def subscribe(
        ...
        logger.info("Subscribed to events of type '%s'", event_type)
    ...
    def unsubscribe(
        ...
        if event_type in self._subscribers:
            try:
                ...
                logger.info("Unsubscribed from events of type '%s'", event_type)
                return True
            except ValueError:
                pass
    ...
    async def _event_listening_loop(self) -> None:
        ...
        try:
            ...
            logger.error("Error in event listening loop: %s", e)
        finally:
            await pubsub.close()
            ...
            try:
                ...
                async with self._buffer_lock:
                    if self._event_buffer:
                        batch_size = min(
                            len(self._event_buffer), self.config.event_batch_size
                        )
                ...
                logger.error("Error in buffer processing loop: %s", e)
    ...
    async def get_event_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "event_types": list(self._subscribers),
...
class EventPublisher:
    ...
    async def publish_service_error(
    ...
    async def publish_custom_event(self, event_type: str, data: Dict[str, Any]) -> int:
