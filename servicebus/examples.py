...
class ExampleEventService:
    ...
    async def start(self):
        ...
        self.server.register_handler(
            "trigger_event", self.trigger_event, "Trigger a custom event"
        )
        self.server.register_handler(
...
async def run_event_example():
    ...
    try:
        ...
        processed = await client.call(
            target_service="event-service", request_type="get_processed_events"
        )
    ...
    try:
        ...
        for op in operations:
            result = await client.call(
                target_service="decorator-service", request_type="calculate", data=op
            )
