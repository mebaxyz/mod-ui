...
async def test_audio_engine_service():
    ...
    try:
        ...
        try:
            ...
            print(f"   ✅ JACK Data: CPU Load={jack_data.cpu_load:.2f}%, Sample Rate={jack_data.sample_rate}Hz")
        ...
        # Test hardware ports
        try:
            audio_in_ports = await service.get_jack_hardware_ports(is_audio=True, is_output=False)
            audio_out_ports = await service.get_jack_hardware_ports(is_audio=True, is_output=True)
            print(f"   ✅ Hardware Ports: {len(audio_in_ports)} inputs, {len(audio_out_ports)} outputs")
...
async def test_fallback_functionality():
    ...
    try:
        from mod_ui.services.audio_engine.jack_lv2_utils import get_jack_manager, get_lv2_manager
        ...
        print(f"JACK Data (fallback): SR={jack_data.sample_rate}Hz, BS={jack_data.buffer_size}")
