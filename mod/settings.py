...
IMAGE_VERSION_PATH = os.environ.pop(
    "MOD_IMAGE_VERSION_PATH", "/etc/mod-release/release"
)
HARDWARE_DESC_FILE = os.environ.pop(
    "MOD_HARDWARE_DESC_FILE", "/etc/mod-hardware-descriptor.json"
)
...
FAVORITES_JSON_FILE = os.environ.get(
    "MOD_FAVORITES_JSON", join(DATA_DIR, "favorites.json")
)
LAST_STATE_JSON_FILE = os.environ.get(
    "MOD_LAST_STATE_JSON", join(DATA_DIR, "last.json")
)
PREFERENCES_JSON_FILE = os.environ.get(
    "MOD_PREFERENCES_JSON", join(DATA_DIR, "prefs.json")
)
USER_ID_JSON_FILE = os.environ.get("MOD_USER_ID_JSON", join(DATA_DIR, "user-id.json"))

USER_BANKS_JSON_FILE = os.environ.get(
    "MOD_USER_BANKS_JSON", join(DATA_DIR, "banks.json")
)
FACTORY_BANKS_JSON_FILE = os.environ.get(
    "MOD_FACTORY_BANKS_JSON", "/usr/share/mod/banks.json"
)
...
PEDALBOARD_TMP_DIR = os.environ.get(
    "MOD_PEDALBOARD_TMP_DIR", join(DATA_DIR, "pedalboard-tmp-data")
)

LV2_PLUGIN_DIR = os.environ.get("MOD_USER_PLUGINS_DIR", os.path.expanduser("~/.lv2"))
LV2_PEDALBOARDS_DIR = os.environ.get(
    "MOD_USER_PEDALBOARDS_DIR", os.path.expanduser("~/.pedalboards")
)
LV2_FACTORY_PEDALBOARDS_DIR = os.environ.get(
    "MOD_FACTORY_PEDALBOARDS_DIR", "/usr/share/mod/pedalboards"
)
...
DEFAULT_PEDALBOARD_COPY = os.environ.pop(
    "MOD_DEFAULT_PEDALBOARD", join(sys.prefix, "share/mod/default.pedalboard")
)
...
# Cloud API addresses
CLOUD_HTTP_ADDRESS = os.environ.pop(
    "MOD_CLOUD_HTTP_ADDRESS", "https://api.mod.audio/v2"
)
CLOUD_LABS_HTTP_ADDRESS = os.environ.pop(
    "MOD_CLOUD_LABS_HTTP_ADDRESS", "https://api-labs.mod.audio/v2"
)
PLUGINS_HTTP_ADDRESS = os.environ.pop(
    "MOD_PLUGINS_HTTP_ADDRESS", "https://pedalboards.mod.audio/plugins"
)
PEDALBOARDS_HTTP_ADDRESS = os.environ.pop(
    "MOD_PEDALBOARDS_HTTP_ADDRESS", "https://pedalboards.mod.audio"
)
PEDALBOARDS_LABS_HTTP_ADDRESS = os.environ.pop(
    "MOD_PEDALBOARDS_LABS_HTTP_ADDRESS", "https://pedalboards-labs.mod.audio"
)
CONTROLCHAIN_HTTP_ADDRESS = os.environ.pop(
...
MIDI_BEAT_CLOCK_SENDER_OUTPUT_PORT = (
    "mclk"  # This is the LV2 symbol of the plug-ins OutputPort
)
...
UPDATE_MOD_OS_FILE = "/data/{}".format(
    os.environ.get("MOD_UPDATE_MOD_OS_FILE", "modduo.tar").replace("*", "cloud")
)
