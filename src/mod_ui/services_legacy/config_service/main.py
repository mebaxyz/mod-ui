...
class ResilientServiceBus:
    ...
    def register_handler(self, method_name: str, handler: Callable):
        ...
        if self._current_service:
            ...
            logger.warning(f"Cannot register handler '{method_name}' - no active service")
...
async def set_setting(key: str, value: Any) -> bool:
    ...
    if success and resilient_bus:
        await resilient_bus.publish_event("settings_changed", {
            "key": key,
            "value": value,
            "timestamp": time.time()
        })
...
async def reset_settings() -> bool:
    ...
    try:
        ...
        # Publish reset event
        if resilient_bus:
            await resilient_bus.publish_event("settings_reset", {
                "timestamp": time.time()
            })
...
async def handle_settings_request(event):
    ...
    try:
        ...
        if request_type == "get_all":
            ...
            # Publish response
            if resilient_bus:
                await resilient_bus.publish_event("settings_response", {
...
async def initialize_enhanced_config_service():
    ...
    try:
        ...
        if resilient_bus._current_service:
            resilient_bus._current_service.register_handler("get_settings", get_settings)
            ...
            resilient_bus._current_service.register_handler("update_settings", update_settings)
            resilient_bus._current_service.register_handler("reset_settings", reset_settings)
