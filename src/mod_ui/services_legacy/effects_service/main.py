...
class ResilientServiceBus:
    ...
    def on_event(self, event_type: str, handler: Callable[[ServiceEvent], Awaitable[None]]) -> None:
        ...
        if self._current_service and self._is_running:
            try:
                ...
                logger.warning(f"Failed to immediately subscribe to '{event_type}': {e}")
    ...
    def register_handler(self, method_name: str, handler: Callable) -> None:
        ...
        if self._current_service and self._is_running:
            try:
                ...
                logger.warning(f"Failed to immediately register method '{method_name}': {e}")
    ...
    async def start(self) -> None:
        ...
        logger.info(f"ResilientServiceBus for '{self.service_name}' started successfully")
    ...
    async def publish_event(self, event_type: str, data: Dict[str, Any]) -> bool:
        ...
        if not self._current_service:
            logger.warning(f"Cannot publish event '{event_type}' - no active connection")
            return False
    ...
    async def _create_connection(self) -> bool:
        try:
            logger.info(f"Creating new ServiceBus connection for '{self.service_name}'...")
...
async def initialize_enhanced_effects_service():
    ...
    try:
        logger.info("🚀 Initializing ENHANCED Effects Service with auto-reconnection...")
        ...
        if resilient_bus.service_instance:
            ...
            handlers = EffectsServiceHandlers(service_instance=resilient_bus.service_instance)
            
            # Register all effect handlers
            resilient_bus.register_handler("effect_add", handlers.handle_effect_add)
            resilient_bus.register_handler("effect_remove", handlers.handle_effect_remove)
            ...
            resilient_bus.register_handler("effect_connect", handlers.handle_effect_connect)
            resilient_bus.register_handler("effect_disconnect", handlers.handle_effect_disconnect)
            resilient_bus.register_handler("effect_parameter_set", handlers.handle_effect_parameter_set)
            resilient_bus.register_handler("effect_parameter_address", handlers.handle_effect_parameter_address)
            resilient_bus.register_handler("effect_preset_load", handlers.handle_effect_preset_load)
            resilient_bus.register_handler("effect_preset_save", handlers.handle_effect_preset_save)
            resilient_bus.register_handler("effect_preset_delete", handlers.handle_effect_preset_delete)
