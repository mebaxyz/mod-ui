...
def take_screenshot(bundle_path, html_dir, cache_dir, size):
    ...
    plugins = pb["plugins"]
    plugins = [
        p for p in plugins if p["uri"] != "http://drobilla.net/ns/ingen#GraphPrototype"
    ]
    plugin_map = {}
    for p in plugins:
        # read plugin data
        ...
        in_ports = (
            data["ports"]["audio"]["input"]
            + data["ports"]["midi"]["input"]
            + data["ports"]["cv"]["input"]
        )
        out_ports = (
            data["ports"]["audio"]["output"]
            + data["ports"]["midi"]["output"]
            + data["ports"]["cv"]["output"]
        )

        if screenshot_path:
            # detect ports and save/read
            version = "{0}.{1}".format(data["version"], data.get("release", 0)).replace(
                ".", "_"
            )
            encoded_uri = base64.b64encode(p["uri"].encode()).decode()
            filename = os.path.join(
                cache_dir,
                "{0}_{1}_{2}_v2".format(
                    __version__.replace(".", "_"), encoded_uri, version
                ),
            )
            validcache = False
            ...
            if not validcache:
                columns = {
                    "in_ports": tuple(
                        tuple(c)
                        for c in detect_first_column(
                            p["uri"], pimg, pimg.size[0], len(in_ports)
                        )
                    ),
                    "out_ports": tuple(
                        tuple(c)
                        for c in detect_first_column(
                            p["uri"], pimg, pimg.size[0], len(out_ports), rtol=True
                        )
                    ),
                }
        ...
        if len(in_ports) > 0:
            ...
            if not all("connector" in p for p in in_ports):
                raise Exception(
                    "Connector detection for input ports of plugin {0} failed".format(
                        p["uri"]
                    )
                )
        ...
        if len(out_ports) > 0:
            ...
            if not all("connector" in p for p in out_ports):
                raise Exception(
                    "Connector detection for output ports of plugin {0} failed".format(
                        p["uri"]
                    )
                )
    ...
    device_capture = tuple(
        ...
        if d["type"] in used_types
        or d["symbol"] in ("serial_midi_in", "midi_merger_out")
        or d["symbol"] in used_symbols
    )
    device_playback = tuple(
        d
        for d in device_playback
        if d["type"] in used_types
        or d["symbol"] in ("serial_midi_out", "midi_broadcaster_in")
        or d["symbol"] in used_symbols
    )
    ...
    for ix, c in enumerate(pb["connections"]):
        ...
        if source:
            ...
            source_pos = anchor(
                source_connected_img.size, source["x"], source["y"], Anchor.LEFT_CENTER
            )
            ...
            try:
                ...
                continue
            all_ports = (
                source["data"]["ports"]["audio"]["output"]
                + source["data"]["ports"]["midi"]["output"]
                + source["data"]["ports"]["cv"]["output"]
            )
            try:
            ...
            source_pos = (
                source["x"] + conn[0][0] - port["offset"][0],
        ...
        if target:
            ...
            target_pos = anchor(
                target_connected_img.size, target["x"], target["y"], Anchor.RIGHT_CENTER
            )
            ...
            all_ports = (
                target["data"]["ports"]["audio"]["input"]
                + target["data"]["ports"]["midi"]["input"]
                + target["data"]["ports"]["cv"]["input"]
            )
            try:
            ...
            target_pos = (
                target["x"] + conn[0][0] - port["offset"][0],
        ...
        connectors.append(
            (
                ...
                (rint(source_pos[0]), rint(source_pos[1])),
                source_connected_img,
            )
        )
        connectors.append(
            (
                ...
                (rint(target_pos[0]), rint(target_pos[1])),
                target_connected_img,
            )
        )
...
def main():
    ...
    def _take_screenshot(args):
        try:
            take_screenshot(
                args.pedalboard_path, args.html_path, args.cache_path, args.size
            )
        except:
            print(
                "caught exception, retrying without cache...",
                file=sys.stderr,
                flush=True,
            )
    ...
    subparser = subparsers.add_parser(
        "take_screenshot", help="Take a screenshot of a given pedalboard"
    )
    ...
    subparser.add_argument(
        "-s", "--size", help="Screenshot width and height (i.e. 800x600)"
    )
