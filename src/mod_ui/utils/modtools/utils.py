...
from ..mod_legacy import get_unique_name
...
def structToDict(struct):
    return dict(
        (attr, toPythonType(getattr(struct, attr), attr))
        for attr, value in struct._fields_
    )
...
utils.set_util_callbacks.argtypes = (
    JackBufSizeChanged,
    JackPortAppeared,
    JackPortDeleted,
    TrueBypassStateChanged,
)
...
def add_bundle_to_lilv_world(bundlepath):
    return charPtrPtrToStringList(
        utils.add_bundle_to_lilv_world(bundlepath.encode("utf-8"))
    )
...
def remove_bundle_from_lilv_world(bundlepath, resource):
    return charPtrPtrToStringList(
        utils.remove_bundle_from_lilv_world(
            bundlepath.encode("utf-8"), resource.encode("utf-8") if resource else None
        )
    )
...
def is_plugin_preset_valid(plugin, preset):
    return bool(
        utils.is_plugin_preset_valid(plugin.encode("utf-8"), preset.encode("utf-8"))
    )
...
def _get_all_pedalboards_factory():
    ...
    if _allfactorypedalboards is None:
        _allfactorypedalboards = structPtrPtrToList(
            utils.get_all_pedalboards(kPedalboardInfoFactoryOnly)
        )
...
def get_all_pedalboards(ptype):
    ...
    if ptype == kPedalboardInfoBoth:
        ...
        if _allpedalboards is None:
            _allpedalboards = (
                _get_all_pedalboards_user() + _get_all_pedalboards_factory()
            )
...
def set_util_callbacks(bufSizeChanged, portAppeared, portDeleted, trueBypassChanged):
    ...
    utils.set_util_callbacks(
        bufSizeChangedCb, portAppearedCb, portDeletedCb, trueBypassChangedCb
    )
