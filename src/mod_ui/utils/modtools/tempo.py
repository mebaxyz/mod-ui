...
dividers = [
    ...
    {"value": 1, "label": "1"},
    {"value": 1.5, "label": "1T"},
    {"value": 1.333, "label": "1/2."},
    {"value": 2, "label": "1/2"},
    {"value": 3, "label": "1/2T"},
    {"value": 2.666, "label": "1/4."},
    {"value": 4, "label": "1/4"},
    {"value": 6, "label": "1/4T"},
    {"value": 5.333, "label": "1/8."},
    {"value": 8, "label": "1/8"},
    {"value": 12, "label": "1/8T"},
    {"value": 10.666, "label": "1/16."},
    {"value": 16, "label": "1/16"},
    {"value": 24, "label": "1/16T"},
    {"value": 21.333, "label": "1/32."},
    {"value": 32, "label": "1/32"},
...
def convert_equivalent(value, conversion_factor, port_unit_symbol):
    ...
    if port_unit_symbol == "s" or port_unit_symbol == "ms" or port_unit_symbol == "min":
        ...
        port_unit_symbol == "Hz"
        or port_unit_symbol == "MHz"
        or port_unit_symbol == "kHz"
    ):
        return conversion_factor / value
    else:
        return None
...
def get_divider_options(port, min_bpm, max_bpm):
    ...
    if port["units"]["symbol"] == "BPM":
        ...
        min_value = convert_port_value_to_seconds_equivalent(
            port["ranges"]["minimum"], port["units"]["symbol"]
        )
        max_value = convert_port_value_to_seconds_equivalent(
            port["ranges"]["maximum"], port["units"]["symbol"]
        )
    ...
    if "hasStrictBounds" in port["properties"]:
        smin = (
            max(s1_min_bpm, s1_max_bpm)
            if s1_min_bpm < s2_min_bpm
            else max(s2_min_bpm, s2_max_bpm)
        )
        smax = (
            min(s2_min_bpm, s2_max_bpm)
            if s1_min_bpm < s2_min_bpm
            else min(s1_min_bpm, s1_max_bpm)
        )
    else:
