...
function Desktop(elements) {
  ...
  this.hardwareManager = new HardwareManager({
    ...
    setEnabled: function (
      ...
      if (!enabled && feedback === undefined) {
        console.warn(
          "ERROR setEnabled called as false, but with undefined feedback"
        );
        feedback = true;
      }
      if (instance == "/pedalboard") {
        self.transportControls.setControlEnabled(
          portSymbol,
          enabled,
          feedback,
          forceAddress,
  ...
  this.pedalPresets = new SnapshotsManager({
    ...
    renamedCallback: function (name) {
      self.titleBox.text(
        (self.title || "Untitled") + " - " + (name || "Default")
      );
    },
  });
  ...
  this.pedalboard = self.makePedalboard(
    elements.pedalboard,
    elements.effectBox
  );
  ...
  this.authenticateDevice = function (callback) {
    // Skip cloud authentication for local development
    console.log("Skipping cloud authentication for local development");
    callback(true, {
      from_args: {
        headers: { Authorization: "MOD local_dev_token" },
      },
    });
    return;

    // Original cloud authentication code (commented out)
    /*
        $.ajax({
            method: 'GET',
  ...
  this.transportControls = new TransportControls({
    ...
    removeBPMHardwareMapping: function (syncMode) {
      ...
      if (self.hardwareManager.removeHardwareMappping(instanceAndSymbol)) {
        ...
        new Notification(
          "info",
          "BPM addressing removed, incompatible with " + source + " sync mode",
          8000
        );
      }
      self.setPedalboardAsModified(true);
    ...
    unaddressPort: function (portSymbol, syncMode, callback) {
      ...
      $.ajax({
        ...
        success: function (resp) {
          if (resp) {
            if (
              self.hardwareManager.removeHardwareMappping(instanceAndSymbol)
            ) {
              var source = syncMode === "link" ? "Ableton Link" : "MIDI";
              new Notification(
                "info",
                "BPM addressing removed, incompatible with " +
                  source +
                  " sync mode",
                8000
              );
  ...
  this.checkHardwareDeviceVersion = function (dev_uri, label, version) {
    ...
    if (compareVersions(version.split("."), cloudversion.split("."), 3) < 0) {
      data = {
        ...
        label: label,
        "download-url":
          CONTROLCHAIN_URL + "/file/" + label + cloudversion + ".bin",
        "release-url":
          "http://wiki.moddevices.com/wiki/Control_Chain_Releases#" +
          label +
          "," +
          cloudversion,
      };
  ...
  this.setupMatomo = function () {
    ...
    (function () {
      ...
      g.async = true;
      g.src =
        "https://cdn.matomo.cloud/modaudio.matomo.cloud/container_DfEOyKDN.js";
      s.parentNode.insertBefore(g, s);
    })();
  };

  this.effectBox = self.makeEffectBox(
    elements.effectBox,
    elements.effectBoxTrigger
  );
  this.cloudPluginBox = self.makeCloudPluginBox(
    elements.cloudPluginBox,
    elements.cloudPluginBoxTrigger
  );
  this.pedalboardBox = self.makePedalboardBox(
    elements.pedalboardBox,
    elements.pedalboardBoxTrigger
  );
  this.bankBox = self.makeBankBox(elements.bankBox, elements.bankBoxTrigger);
  this.fileManagerBox = self.makeFileManagerBox(
    elements.fileManagerBox,
  ...
  (this.installMissingPlugins = function (plugins, callback) {
    if (self.isApp) {
      new Notification(
        "warn",
        "Cannot load this pedalboard, some plugins are missing",
        4000
      );
      callback(false);
      return;
    ...
    var finalCallback = function () {
      ...
      if (
        error &&
        !confirm(
          "Failed to install some required plugins, do you want to load the pedalboard anyway?"
        )
      ) {
        callback(false);
        return;
    ...
    var installPlugin = function (uri, data) {
      ...
      self.installationQueue.installUsingURI(
        uri,
        "auto",
        function (resp, bundlename) {
          if (!resp.ok) {
            error = true;
          }
    ...
    var installMissing = function (data) {
      for (var i in uris) {
        ...
        if (localplugin == null) {
          ...
          var version = [
            localplugin.builder || 0,
            localplugin.minorVersion,
            localplugin.microVersion,
            localplugin.release || 0,
          ];
    ...
    (this.loadRemotePedalboard = function (pedalboard_id) {
      ...
      if (self.cloudAccessToken == null) {
        self.authenticateDevice(function (ok) {
          if (ok && self.cloudAccessToken != null) {
            ...
            new Notification(
              "error",
              "Cannot load remote pedalboards, authentication failure"
            );
          }
        });
        return;
      ...
      $.ajax({
        url: startsWith(pedalboard_id, "https://")
          ? pedalboard_id
          : SITEURL + "/pedalboards/" + pedalboard_id,
        contentType: "application/json",
        success: function (resp) {
          if (
            !resp.data.stable &&
            PREFERENCES["show-labs-plugins"] !== "true"
          ) {
            new Notification(
              "error",
              'This pedalboard contains one or more community maintained MOD Labs plugins. To load it, you need to enable MOD Labs plugins in <a href="settings">Settings</a> -> Advanced'
            );
            return;
          }
          self.reset(function () {
        ...
        error: function (resp) {
          new Bug(
            "Couldn't get pedalboard info, error:<br/>" + resp.statusText
          );
        },
        cache: false,
        dataType: "json",
  ...
  elements.snapshotSaveAsButton.click(function () {
    var addressed =
      !!self.hardwareManager.addressingsByPortSymbol["/pedalboard/:presets"];
    if (addressed) {
      return new Notification(
        "warn",
        "Cannot change snapshot while addressed to hardware",
        3000
      );
    }
  ...
  elements.snapshotManageButton.click(function () {
    ...
    if (addressed) {
      ...
      feedback =
        self.hardwareManager.addressingsData["/pedalboard/:presets"].feedback;
    }
  ...
  elements.shareButton.click(function () {
    var share = function () {
      ...
      if (uris.length == 0) {
        return new Notification(
          "warn",
          "No plugins loaded, cannot share",
          1500
        );
      }
    ...
    if (
      self.pedalboardModified ||
      !(self.pedalboardEmpty || self.pedalboardBundle)
    ) {
      if (
        confirm(
          "There are unsaved modifications, pedalboard must first be saved. Save it?"
        )
      ) {
  ...
  elements.shareWindow.shareBox({
    recordStart: ajaxFactory(
      "/recording/start",
      "Can't record. Probably a connection problem."
    ),
    recordStop: ajaxFactory(
      "/recording/stop",
      "Can't stop record. Probably a connection problem. Please try stopping again"
    ),
    ...
    playStop: ajaxFactory(
      "/recording/play/stop",
      "Can't stop playing. Probably a connection problem. Please try stopping again"
    ),
    recordDownload: ajaxFactory(
      "/recording/download",
      "Can't download recording. Probably a connection problem."
    ),
    recordReset: ajaxFactory(
      "/recording/reset",
      "Can't reset your recording. Probably a connection problem."
    ),
    ...
    share: function (data, callback) {
      ...
      $.ajax({
        ...
        success: function (resp) {
          var transfer = new SimpleTransference(
            "/pedalboard/pack_bundle/?bundlepath=" +
              escape(self.pedalboardBundle),
            resp.upload_href,
            {
              to_args: {
                headers: { Authorization: "MOD " + self.cloudAccessToken },
          ...
          transfer.reportError = function (error) {
            callback({
              ok: false,
              error:
                "Failed to upload pedalboard to cloud (missing screenshot?)",
            });
          };
        ...
        error: function (resp) {
          if (resp.status == 401 && !data.reauthorized) {
            console.log(
              "Pedalboard share unauthorized, retrying authentication..."
            );
  ...
  el.pedalboard({
      ...
      var add = function () {
        $.ajax({
          ...
          error: function (resp) {
            new Notification(
              "error",
              "Error adding effect. Probably a connection problem."
            );
            if (errorCallback) errorCallback();
          },
          cache: false,
    ...
    pluginRemove: function (instance, callback) {
      $.ajax({
        ...
        success: function (resp) {
          if (resp) callback();
          else new Notification("error", "Couldn't remove effect");
        },
        cache: false,
        dataType: "json",
      });
    ...
    pluginPresetSaveReplace: function (
      instance,
      uri,
      bundlepath,
      name,
    ...
    pluginPatchSet: function (instance, uri, valuetype, value) {
      self.setPedalboardAsModified(true);
      ws.send(
        sprintf("patch_set %s %s %s %s", instance, uri, valuetype, value)
      );
    },
    ...
    windowSize: function (width, height) {
      // FIXME
      if (typeof ws !== "undefined" && ws && width > 0 && height > 0) {
    ...
    notifyDemoPluginLoaded: function () {
      ...
      new Notification(
        "warn",
        "This pedalboard contains a trial plugin.<br>Using trial plugins will intentionally mute the audio at regular intervals."
      );
    },
  });
  ...
  return el.pedalboardBox({
    ...
    remove: function (pedalboard, callback) {
      if (
        !confirm(
          sprintf(
            'The pedalboard "%s" will be permanently removed! Confirm?',
            pedalboard.title
          )
        )
      )
      ...
      $.ajax({
        ...
        success: function () {
          new Notification(
            "info",
            sprintf('Pedalboard "%s" removed', pedalboard.title),
            1000
          );
          self.previousPedalboardList = null;
          callback();
...
Desktop.prototype.makeCloudPluginBox = function (el, trigger) {
  ...
  return el.cloudPluginBox({
    ...
    removePluginBundles: function (bundles, callback) {
      if (
        !confirm(
          "You are about to remove this plugin and any other in the same bundle. This may break pedalboards that depend on them."
        )
      )
        return;
      self.previousPedalboardList = null;
      $.ajax({
        ...
        success: function (resp) {
          if (resp.ok) {
            ...
            new Notification(
              "error",
              "Could not uninstall bundle: " + resp.error
            );
          }
        },
        error: function () {
      ...
      $.ajax({
        ...
        success: function (resp) {
          if (resp) {
            ...
            new Notification(
              "warning",
              "Factory pedalboard was duplicated into the User Pedalboards Library"
            );
            callback(resp);
          } else {
...
Desktop.prototype.reset = function (callback, skipConfirmDialog) {
  if (
    this.pedalboardModified &&
    !skipConfirmDialog &&
    !confirm("There are unsaved modifications that will be lost. Are you sure?")
  ) {
    return;
  }
...
Desktop.prototype.openPresetSaveWindow = function (
  windowTitle,
  name,
  callback
) {
  this.presetSaveBox.saveBox(
    "save",
    windowTitle,
    name,
    true,
...
JqueryClass("saveBox", {
  init: function (options) {
    ...
    self.find("input").keyup(function () {
      self
        .find(".js-save")
        .prop("disabled", this.value.length == 0 ? true : false);
    });
  ...
  edit: function () {
    ...
    self
      .find(".js-save")
      .prop("disabled", self.find("input").val().length == 0 ? true : false);
    self.show();
    self.focus();
  ...
  send: function () {
    ...
    self.data("save")("", title, asNew, function (ok, errorOrPath, realTitle) {
      if (!ok) {
        new Bug(errorOrPath);
      }
...
JqueryClass("statusTooltip", {
  ...
  status: function (status) {
    var self = $(this);
    if (self.data("status")) self.removeClass(self.data("status"));
  ...
  updatePosition: function () {
    ...
    tooltip.css(
      "right",
      $(window).width() - self.position().left - self.width()
    );
