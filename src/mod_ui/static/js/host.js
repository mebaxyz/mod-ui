...
$("document").ready(function () {
  ...
  ws.onmessage = function (evt) {
    ...
    cmd = cmd[0];
    console.log(
      "DEBUG: Processing WebSocket command:",
      cmd,
      "with data:",
      data
    );
    ...
    if (cmd == "stats") {
      ...
      if (cpuLoad != cached_cpuLoad) {
        ...
        $("#cpu-bar").css(
          "width",
          (100.0 - cpuLoad).toFixed().toString() + "%"
        );
      ...
      if (xruns != cached_xruns) {
        cached_xruns = xruns;
        $("#mod-xruns").text(
          xruns == 1 ? xruns.toString() + " Xrun" : xruns.toString() + " Xruns"
        );
    ...
    if (cmd == "sys_stats") {
      ...
      if (cpufreq !== "0" && cputemp !== "0") {
        ...
        $("#mod-cpu-stats").html(
          sprintf("%.1f GHz", parseInt(cpufreq) / 1000000)
        );
      } else if (cputemp !== "0") {
        $("#mod-cpu-stats").html(
          sprintf("%d &deg;C", parseInt(cputemp) / 1000)
        );
      }
      return;
    }
    ...
    if (cmd == "hw_map") {
      ...
      desktop.hardwareManager.addHardwareMapping(
        instance,
        symbol,
        actuator,
        label,
    ...
    if (cmd == "cv_map") {
      ...
      desktop.hardwareManager.addCvMapping(
        instance,
        symbol,
        actuator,
        label,
    ...
    if (cmd == "midi_map") {
      ...
      if (channel < 0 || control < 0 || minimum >= maximum) {
        console.log(
          "WARNING: Received MIDI mapping with invalid values, ignored"
        );
        return;
      }

      desktop.hardwareManager.addMidiMapping(
        instance,
        symbol,
        channel,
        control,
      ...
      if (!connMgr.connected(source, target)) {
        ...
        if (output.length) {
          ...
          if (input.length) {
            desktop.pedalboard.pedalboard(
              "connect",
              output.find("[mod-role=output-jack]"),
              input,
              skipModified
            );
          } else {
          ...
          success: function (pluginData) {
            ...
            if (!$(instancekey).length) {
              var cb = function () {
                ...
                desktop.pedalboard
                  .data("wait")
                  .stopPlugin(instance, !skipModified);
    ...
    if (cmd == "add_hw_port") {
      ...
      if (isOutput) {
        ...
        var el = $(
          '<div id="' +
            instance +
            '" class="hardware-output" mod-port-index=' +
            index +
            ' title="Hardware ' +
            name +
            '">'
        ...
        var el = $(
          '<div id="' +
            instance +
            '" class="hardware-input" mod-port-index=' +
            index +
            ' title="' +
            prefix +
            " " +
    ...
    if (cmd == "loading_end") {
      console.log("DEBUG: Received loading_end command with data:", data);

      // IMMEDIATE FIX: Initialize the main UI immediately
      console.log("DEBUG: Adding initialized class directly");
      ...
      $.ajax({
        ...
        success: function (resp) {
          ...
          if (resp.ok) {
            desktop.titleBox.text(
              (desktop.title || "Untitled") + " - " + resp.name
            );
          }

          pb_loading = false;
          // Stop the pedalboard loading spinner
          desktop.pedalboard.data("wait").stop();
        },
        error: function (xhr, status, error) {
          console.error(
            "DEBUG: Snapshot API error:",
            status,
            error,
            xhr.responseText
          );
          pb_loading = false;
          // Stop the pedalboard loading spinner even on error
          desktop.pedalboard.data("wait").stop();
        },
        cache: false,
        dataType: "json",
