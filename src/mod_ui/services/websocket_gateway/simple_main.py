...
async def list_clients():
    ...
    return {
        "success": True,
        "clients": [
            {"client_id": client_id} for client_id in active_connections.keys()
        ],
...
async def send_transport_message(
    rolling: bool = False, bpb: float = 4.0, bpm: float = 120.0, sync: str = "none"
) -> int:
...
async def send_stats(stats_data: Dict[str, Any]):
    ...
    return {
        "success": True,
        "message": "Stats sent",
        "clients_reached": len(active_connections),
    }
        ...
        while True:
            try:
                ...
                if data.startswith("data_ready "):
                    ...
                    logger.info(
                        f"Frontend responded with data_ready, sending initialization sequence"
                    )
