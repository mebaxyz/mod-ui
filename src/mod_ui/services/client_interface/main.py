...
async def get_pedalboards():
    ...
    try:
        response = await service_bus.request(
            "system_resource_management", "get_pedalboards", {}
        )
        return response
    except Exception as e:
...
async def create_pedalboard(pedalboard: PedalboardRequest):
    ...
    try:
        response = await service_bus.request(
            "audio_processing", "create_pedalboard", pedalboard.dict()
        )
        await connection_manager.broadcast(
            json.dumps({"type": "pedalboard_created", "data": response})
        )
        return response
    except Exception as e:
...
async def update_pedalboard(pedalboard_id: str, pedalboard: PedalboardRequest):
    ...
    try:
        ...
        response = await service_bus.request(
            "audio_processing", "update_pedalboard", data
        )
        await connection_manager.broadcast(
            json.dumps({"type": "pedalboard_updated", "data": response})
        )
        return response
    except Exception as e:
...
async def delete_pedalboard(pedalboard_id: str):
    ...
    try:
        ...
        await connection_manager.broadcast(
            json.dumps(
                {"type": "pedalboard_deleted", "data": {"pedalboard_id": pedalboard_id}}
            )
        )
        return response
    except Exception as e:
...
async def add_plugin(plugin: PluginRequest):
    ...
    try:
        response = await service_bus.request(
            "audio_processing", "add_plugin", plugin.dict()
        )
        await connection_manager.broadcast(
            json.dumps({"type": "plugin_added", "data": response})
        )
        return response
    except Exception as e:
...
async def remove_plugin(instance_id: str):
    ...
    try:
        response = await service_bus.request(
            "audio_processing", "remove_plugin", {"instance_id": instance_id}
        )
        await connection_manager.broadcast(
            json.dumps({"type": "plugin_removed", "data": {"instance_id": instance_id}})
        )
        return response
    except Exception as e:
...
async def update_parameter(param_update: ParameterUpdate):
    ...
    try:
        response = await service_bus.request(
            "audio_processing", "update_parameter", param_update.dict()
        )
        await connection_manager.broadcast(
            json.dumps({"type": "parameter_updated", "data": param_update.dict()})
        )
        return response
    except Exception as e:
...
async def get_system_status():
    ...
    try:
        response = await service_bus.request(
            "system_resource_management", "get_system_status", {}
        )
        return response
    except Exception as e:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_text(
            json.dumps(
                {"type": "connection_established", "data": {"service": SERVICE_NAME}}
            )
        )
    ...
    try:
        if message_type == "parameter_update":
            ...
            if service_bus:
                ...
                await connection_manager.broadcast(
                    json.dumps({"type": "parameter_updated", "data": data})
                )

        elif message_type == "get_status":
            # Send current status
            if service_bus:
                status = await service_bus.request(
                    "system_resource_management", "get_system_status", {}
                )
                await websocket.send_text(
