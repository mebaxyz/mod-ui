...
from .models import JackConnectionInfo, JackData, JackPortInfo, LV2PluginInfo, LV2PluginPort
...
class JackManager:
    ...
    def _get_jack_data_fallback(self) -> JackData:
        ...
        try:
            ...
            sample_rate = float(result.stdout.strip()) if result.returncode == 0 else 48000.0
    ...
    def get_hardware_ports(self, is_audio: bool = True, is_output: bool = False) -> List[JackPortInfo]:
    ...
    def _get_hardware_ports_fallback(self, is_audio: bool, is_output: bool) -> List[JackPortInfo]:
        ...
        try:
            ...
            for line in result.stdout.strip().split('\n'):
                if line and 'system:' in line:  # Hardware ports typically start with system:
                    ports.append(JackPortInfo(
                        name=line.strip(),
                        is_audio=is_audio,
                        is_output=is_output,
                    ))
    ...
    def disconnect_ports(self, output_port: str, input_port: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to disconnect ports {output_port} -> {input_port}: {e}")
            return False
...
class LV2Manager:
    ...
    def get_all_plugins(self) -> List[Dict[str, Any]]:
        ...
        try:
            if HAVE_MOD_UTILS:
                ...
                return [self._convert_plugin_mini_to_dict(p) for p in plugins] if plugins else []
            else:
                # Fallback: get basic info for each plugin
                plugin_uris = self.get_plugin_list()
                return [{"uri": uri, "name": uri.split('/')[-1]} for uri in plugin_uris[:50]]  # Limit for performance
        except Exception as e:
    ...
    def _get_plugin_info_fallback(self, plugin_uri: str) -> Optional[LV2PluginInfo]:
        ...
        try:
            ...
            logger.warning(f"Fallback plugin info retrieval failed for {plugin_uri}: {e}")
            return None
    ...
    def remove_bundle(self, bundle_path: str, resource: Optional[str] = None) -> List[str]:
