...
async def execute_system_command(command: SystemCommand):
    ...
    if command.command not in allowed_commands:
        raise HTTPException(status_code=403, detail=f"Command '{command.command}' not allowed")
...
async def file_operation(operation: FileOperation):
    ...
    try:
        ...
        elif operation.operation == "move" and operation.source and operation.destination:
    ...
    while True:
        try:
            ...
            if service_bus:
                ...
                if cpu_usage > 90:
                    await service_bus.publish("system_alert", {
                        "type": "high_cpu",
                        "value": cpu_usage,
                        "threshold": 90
                    })
