...
def mock_connection_manager():
    ...
    manager.get_stats = AsyncMock(return_value=Mock(
...
def mock_event_router():
    ...
    router.get_stats = AsyncMock(return_value=Mock(
        events_processed=50,
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if "test_" in item.name and not any(mark.name in ["integration", "slow"] for mark in item.own_markers):
