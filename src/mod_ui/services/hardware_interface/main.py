...
class HardwareManager:
    ...
    def _init_simulated_hardware(self):
        ...
        self.controls = {
            "encoder_1": HardwareControl("encoder_1", ControlType.ENCODER, "Main Encoder", {"x": 64, "y": 32}),
            "button_1": HardwareControl("button_1", ControlType.BUTTON, "Select Button", {"x": 64, "y": 50}),
            "button_2": HardwareControl("button_2", ControlType.BUTTON, "Menu Button", {"x": 32, "y": 50}),
            "button_3": HardwareControl("button_3", ControlType.BUTTON, "Back Button", {"x": 96, "y": 50}),
            "footswitch_1": HardwareControl("footswitch_1", ControlType.FOOTSWITCH, "Footswitch 1", {"x": 20, "y": 80}),
            "footswitch_2": HardwareControl("footswitch_2", ControlType.FOOTSWITCH, "Footswitch 2", {"x": 108, "y": 80}),
            "expression": HardwareControl("expression", ControlType.EXPRESSION_PEDAL, "Expression Pedal", {"x": 64, "y": 100})
        }
    ...
    def _init_controls(self):
        ...
        if not SIMULATE_HARDWARE:
            ...
            controls_config = {
                "encoder_main": HardwareControl("encoder_main", ControlType.ENCODER, "Main Encoder", {"x": 64, "y": 32}),
                "button_select": HardwareControl("button_select", ControlType.BUTTON, "Select", {"x": 64, "y": 50}),
                "button_menu": HardwareControl("button_menu", ControlType.BUTTON, "Menu", {"x": 32, "y": 50}),
                "button_back": HardwareControl("button_back", ControlType.BUTTON, "Back", {"x": 96, "y": 50}),
                "footswitch_1": HardwareControl("footswitch_1", ControlType.FOOTSWITCH, "FS1", {"x": 20, "y": 80}),
                "footswitch_2": HardwareControl("footswitch_2", ControlType.FOOTSWITCH, "FS2", {"x": 108, "y": 80}),
                "expression": HardwareControl("expression", ControlType.EXPRESSION_PEDAL, "Expression", {"x": 64, "y": 100})
            }
        else:
    ...
    async def _handle_midi_cc(self, cc_number: int, value: int):
        ...
        for control_id, control in hardware_controls.items():
            if control.midi_cc == cc_number:
                ...
                normalized = (value / 127.0) * (control.max_value - control.min_value) + control.min_value
                control.value = normalized
                
                # Send parameter update if assigned
                if control.assignment and service_bus:
                    await service_bus.request("audio_processing", "update_parameter", {
    ...
    async def simulate_control_change(self, control_id: str, value: float):
        ...
        if SIMULATE_HARDWARE and control_id in hardware_controls:
            ...
            # Trigger callbacks
            if service_bus:
                await service_bus.publish("hardware_event", {
                    "type": "control_change",
...
async def get_hardware_controls():
    ...
    return {
        "controls": {k: asdict(v) for k, v in hardware_controls.items()}
    }
...
async def update_midi_config(config: MidiConfig):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to update MIDI configuration")
